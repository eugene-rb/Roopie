...
最新: パスワードマネージャーとオートフィル(住所・クレカ)をChrome並みに拡充(候補ドロップダウン/パスワード生成/除外リスト/CSV入出力/設定画面「自動入力」)。※「拡張機能パズルボタン」はWIPコミット(9c3c8ce、メニューCSS未整備)
次の作業(ユーザー指示 2026-07-17、順に): ①スタート画面ウィジェット(天気/ノートパッド/カレンダー/ニュース、ショートカットと合わせグリッド並べ替え) ②プロファイル切り替えをEdge挙動に(新しいウィンドウを開く)
