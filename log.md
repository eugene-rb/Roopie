...
- **サイドパネル**(F4 / ツールバーの◨ボタン。Vivaldiのパネル準拠): アイコンレールの並びはブックマーク→ダウンロード→履歴→メモ→リーディングリスト→ピン留めWebパネル→「+」(ウェブパネル追加)。自動保存メモ、Webパネル(任意サイトの常駐表示・複数登録)。同じアイコンをもう一度押すと折りたたみ(Vivaldi同様)。**Webパネルの管理画面は無し**: 追加は「+」またはレール右クリック、編集(名前/アイコン/URL)・削除はピン留めアイコンの右クリックメニューから。データはプロファイル単位。※AIアシスタント機能は2026-07-17に廃止
