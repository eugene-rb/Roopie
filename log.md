...
- **サイドパネル**(F4 / ツールバーの◨ボタン。Vivaldiのパネル準拠): アイコンレールの並びはブックマーク→ダウンロード→履歴→メモ→リーディングリスト→Webパネル管理→ピン留めWebパネル→「+」(ウェブパネル追加)。自動保存メモ、Webパネル(任意サイトの常駐表示・複数登録)。同じアイコンをもう一度押すと折りたたみ(Vivaldi同様)。データはプロファイル単位。※AIアシスタント機能は2026-07-17に廃止
