...
最新: プロファイル切り替えをEdge挙動に(切り替え=新しいウィンドウ、複数プロファイル同時利用。ウィンドウ単位のプロファイルにアーキテクチャ変更 324e356)。同日にパスワード/オートフィル拡充、スタート画面ウィジェットも実装。※「拡張機能パズルボタン」はWIPコミット(9c3c8ce、メニューCSS未整備)
...
- **スタート画面ウィジェット**: 「+」から天気(Open-Meteo・都市名検索)/ノートパッド(自動保存)/カレンダー(月表示)/ニュース(RSS登録、NHK・Yahoo!プリセット)を追加。ショートカットと同じグリッドでドラッグ並べ替え(スマホのホーム画面風、ページ単位で保存)。⋮メニューで設定・更新・削除。検証: `npx electron scripts/test-newtab-widgets.js`
