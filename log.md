...
最新: スタートページのスワイプ切り替え+デフォルトアイコン拡大+共通アイコンピッカーのはみ出し修正(d8ebf71)。直前はスタート画面グリッドの列数・行数をAndroidホーム画面風に設定画面から変更可能に(0fbad81)。さらに前はプロファイル切り替えをEdge挙動に(切り替え=新しいウィンドウ、複数プロファイル同時利用。ウィンドウ単位のプロファイルにアーキテクチャ変更 324e356)。同日にパスワード/オートフィル拡充、スタート画面ウィジェットも実装。※「拡張機能パズルボタン」はWIPコミット(9c3c8ce、メニューCSS未整備)
...
- **スタート画面ウィジェット**: 「+」から天気(Open-Meteo・都市名検索)/ノートパッド(自動保存)/カレンダー(月表示)/ニュース(RSS登録、NHK・Yahoo!プリセット)を追加。ショートカットと同じグリッドでドラッグ並べ替え(スマホのホーム画面風、ページ単位で保存)。⋮メニューで設定・更新・削除。**グリッドの列数(4〜10)・行数(3〜8、既定3)は設定画面「スタート画面」で変更可**(Androidのホーム画面風。セルサイズは画面幅・高さに合わせて自動伸縮、行数を増やして画面が狭い場合はセル縮小→時計位置の上方向オフセット緩和の順で安全弁が効き時計が画面外に出ない)。**複数ページはトラックパッドの横スワイプ/タッチスワイプでも切り替え可能**(スライド+フェードのアニメーション、ドットクリックと併用可)。検証: `npx electron scripts/test-newtab-widgets.js`
