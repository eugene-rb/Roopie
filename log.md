...
最新: スタート画面ウィジェット(天気/ノートパッド/カレンダー/ニュース)+ショートカットとの統合グリッド並べ替え。その前にパスワードマネージャー/オートフィル拡充。※「拡張機能パズルボタン」はWIPコミット(9c3c8ce、メニューCSS未整備)
次の作業(ユーザー指示 2026-07-17): プロファイル切り替えをEdge挙動に(新しいウィンドウを開く)
