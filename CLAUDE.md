...
- ユーザーに求められた改善は逐一ここに追記し、必ず次回以降に聞き継ぐ
  ...
  - (2026-07-25) 裏タブの自動再生対策は「**そもそも読み込まない**」(`hibernated`)の一本だけ。読み込まれたタブの再生には一切干渉しない(勝手に止めない・勝手にミュートしない)。過去に試して外したもの: `webPreferences.autoplayPolicy: 'document-user-activation-required'`(user activation はドキュメント単位でナビゲーションのたびに捨てられるので、**自分で押した再読み込みですら**止まったままになる)、`autoPauseMedia`(裏タブを読み込まなくした結果、発動するのが「ユーザーが切り替えた直後のタブ」だけになり、自分で開いたタブが約1秒鳴ってから止まる挙動になった)。どちらも復活させない
