...
- **プロファイルアイコン**: 設定画面でアバターをクリックすると選択パネル(既定の絵文字24種+自由入力+画像アップロード+文字に戻す)。ツールバー・プロファイルメニュー・設定画面すべてに反映
...
- メディアプレイヤーは未実装。拡張機能の管理UI(一覧・削除・ツールバーボタン)も未実装
...
1. メディアプレイヤー(要件定義書 4.3): フローティング/PinP
2. 拡張機能の管理UI(設定画面に一覧・削除、ツールバーの拡張ボタン=`electron-chrome-extensions` の browser-action UI)
3. サイドパネルの残り機能(AIチャット、カレンダー/TODO、リードリスト。要件定義書 4.2)
4. デザインの継続改善(Preline UIのコンポーネントパターン適用を広げる)
5. 画面分割の発展: ドラッグ&ドロップでの分割(タブを別タブへドロップ)、ペイン間リサイズ、非アクティブペインからの戻る/進む/ズーム操作
