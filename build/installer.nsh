...
!macro customUnInstall
  ; 既定のブラウザとしての登録(アプリが起動時にHKCUへ書く)を消す。
  ; 自動アップデートでもアンインストーラーは走るので、更新時(--updated)は消さない
  ; (消すと更新のたびにRoopieが「既定のアプリ」から居なくなってしまう)
  ${ifNot} ${isUpdated}
    DeleteRegKey HKCU "Software\Clients\StartMenuInternet\Roopie"
    DeleteRegKey HKCU "Software\Classes\RoopieHTML"
    DeleteRegValue HKCU "Software\RegisteredApplications" "Roopie"
  ${endIf}
