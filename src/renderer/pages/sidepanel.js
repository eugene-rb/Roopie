...
const actShutdownNotifyRow = $('act-shutdown-notify-row');
const actShutdownNotify = $('act-shutdown-notify');
...
actShutdown.addEventListener('change', () => {
  ...
  actShutdownNotifyRow.classList.toggle('hidden', !actShutdown.checked);
...
function openTimerModal(id, type) {
  ...
  actShutdownConfirmRow.classList.toggle('hidden', !actShutdown.checked);
  // 未設定(既存タイマー・新規)は通知あり=15秒の猶予つきを既定にする
  actShutdownNotify.checked = actions.notifyBeforeShutdown !== false;
  actShutdownNotifyRow.classList.toggle('hidden', !actShutdown.checked);
...
function applyTimerEdit() {
  ...
  if (timerEditType !== 'stopwatch') {
    ...
    payload.actions = {
      ...
      notifyBeforeShutdown: actShutdownNotify.checked,
    };
  }
