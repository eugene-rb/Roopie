...
function applyGridMetrics() {
  ...
  computedCols = cols;
  // 開いているフォルダはbody直下のオーバーレイで --cell を継承できないため、同じ値を渡す
  document.querySelector('.folder-view')?.style.setProperty('--cell', `${cellPx}px`);
...
let shortcuts = [];
// ページの中に作ったフォルダ(ブックマークのフォルダ)の中身。フォルダID → 中のリンク。
// 展開するのは1階層だけ(フォルダの中のフォルダは持たない)
let folderContents = new Map();

// ブックマークのフォルダ = スマホのアプリフォルダとして扱う項目(URLを持たない)
...
function tileIconContent(tile, shortcut, iconOverride) {
  ...
  // ページの中のフォルダの既定は「中身のアイコンを2x2で並べたプレビュー」。
  // カスタムアイコンを付けたときは普通のタイルと同じ見せ方に戻す
  tile.classList.toggle('group-tile', isGroup(shortcut) && !icon);
  ...
  } else if (isGroup(shortcut)) {
    for (const child of (folderContents.get(shortcut.id) ?? []).slice(0, GROUP_PREVIEW_MAX)) {
      const cell = document.createElement('div');
      cell.className = 'group-mini';
      tileIconContent(cell, child, undefined);
...
function reconcileLayout(rawLayout, list) {
  ...
  // 個数の上限は設けない(表示しきれない分はグリッドを縦スクロールして届く)
...
// ---- フォルダ(スマホのアプリフォルダ風。中身を小さく並べたタイル → タップで展開) ----
const GROUP_PREVIEW_MAX = 4; // タイルに並べる中身のアイコン数(2x2)

function groupItemEl(folder) {
  const count = (folderContents.get(folder.id) ?? []).length;
...
// その場改名(ページのピルと同じ流儀。ページ・フォルダとも bookmarks:update-item を使う)
function beginInlineRename(el, item, onDone) {
  ...
  const commit = (save) => {
    ...
    if (save && title && title !== item.title) {
      window.roopieInternal.updateShortcut(item.id, { title });
      item.title = title; // 変更通知を待たずに表示を合わせる
    }
    onDone();
  };
  input.addEventListener('keydown', (e) => {
    e.stopPropagation(); // ポップアップ・フォルダ表示のEscapeなどに拾わせない
...
function openFolderView(folderId) {
  ...
  backdrop.className = 'folder-view-backdrop';
  backdrop.addEventListener('click', (e) => {
    if (e.target === backdrop) closeFolderView();
  });

  const panel = document.createElement('div');
  panel.className = 'folder-view';
  ...
  document.addEventListener('keydown', onFolderViewKey);
  renderFolderView();
}

function onFolderViewKey(e) {
  if (e.key !== 'Escape') return;
  // 上に開いているもの(編集モーダル・アイコンピッカー・ポップアップ)が先に閉じる
  if (document.querySelector('.shortcut-modal, .icon-picker, .grid-popup')) return;
  closeFolderView();
}

function closeFolderView() {
  openFolderId = null;
  document.querySelector('.folder-view-backdrop')?.remove();
  document.removeEventListener('keydown', onFolderViewKey);
}

// 開いているフォルダの中身を描き直す(ブックマーク側の変更通知でも呼ばれる)。
// フォルダが消えていたら閉じる
function renderFolderView() {
  const backdrop = document.querySelector('.folder-view-backdrop');
  if (!backdrop || !openFolderId) return;
  const folder = shortcuts.find((s) => s.id === openFolderId);
  if (!folder) {
    closeFolderView();
    return;
  }

  // --cell は #quick-links に載るのでbody直下のオーバーレイには継承されない。ここで同じ値を渡す
  const panel = backdrop.querySelector('.folder-view');
  panel.style.setProperty('--cell', `${currentCellPx()}px`);

  const titleEl = backdrop.querySelector('.folder-view-title');
  if (!titleEl.querySelector('input')) {
    titleEl.textContent = folder.title;
    titleEl.ondblclick = () => beginInlineRename(titleEl, folder, () => renderFolderView());
  }
  ...
  if (!contents.length) {
    ...
    empty.textContent = 'このフォルダは空です(ブックマークのパネルからリンクを入れられます)';
...
function shortcutItemEl(shortcut) {
  if (isGroup(shortcut)) return groupItemEl(shortcut);
...
document.addEventListener('contextmenu', (e) => {
  ...
  if (e.target.closest('#search, .grid-item, .grid-popup, .icon-picker, .icon-picker-backdrop, .folder-view-backdrop'))
    return;
  e.preventDefault();
...
// 各ページは名前を持つ。以前は無地のドットだけだったので「どのページか」が分からず、
// 8pxの点しか押せなかった。名前のピルを並べて、クリックで切り替え・
// ダブルクリック(または右クリックメニュー)で改名・右クリックメニューで削除できるようにする
function renderPageBar() {
  ...
  for (const page of pages) {
    ...
    tab.title = `${page.title}(ダブルクリックで名前を変更)`;
    tab.dataset.pageId = page.id;
    tab.addEventListener('click', () => switchToPage(page.id));
    tab.addEventListener('dblclick', () => beginPageRename(tab, page));
    tab.addEventListener('contextmenu', (e) => {
      e.preventDefault();
      openPageMenu({ x: e.clientX, y: e.clientY }, page);
    });
    pageDotsEl.appendChild(tab);
  }
  ...
  pageDotsEl.appendChild(addBtn);
}

// ピルの中でその場改名する(サイドパネルのブックマークツリーと同じ流儀。
// フォルダ名の改名と同じ入力欄を使う)
function beginPageRename(tabEl, page) {
  beginInlineRename(tabEl, page, renderPageBar);
}
  ...
  popupMenu(anchor, [
    {
      label: '✏️ 名前を変更',
      action: () => {
        const tab = pageDotsEl.querySelector(`[data-page-id="${page.id}"]`);
        if (tab) beginPageRename(tab, page);
      },
    },
    {
      label: '🗑 このページを削除',
      disabled: pages.length < 2, // 最後の1ページは残す
      // ショートカットとウィジェットも一緒に消えるので、もう一段確認を挟む
      action: () =>
        popupMenu(anchor, [
          {
            label: `⚠ 「${page.title}」を中身ごと削除`,
            action: async () => {
              window.roopieInternal.removeShortcut(page.id);
              await loadPages(); // 現在のページを消した場合は先頭のページへ戻る
            },
          },
        ]),
    },
...
async function loadShortcuts() {
  ...
  // ページ直下のリンクは、ブックマークのパネル/管理画面から追加したものも含めて全てショートカットとして出す。
  // ページの中のフォルダはスマホのアプリフォルダ風のタイルにする(中身は展開して持っておく)
  ...
  const folders = list.filter(isGroup);
  const contents = await Promise.all(folders.map((f) => window.roopieInternal.listShortcuts(f.id)));
  // フォルダの中のフォルダは出さない(展開するのは1階層だけ)
  folderContents = new Map(folders.map((f, i) => [f.id, (contents[i] ?? []).filter((b) => !isGroup(b))]));
...
// アニメーションの終了待ち。タブが裏に回っていると進行しないので待ち時間で頭打ちにする
// (finishedを素で待つと二度と解決せず、pageSwitchBusyが立ったまま切り替え不能になる)
const settleAnim = (anim, ms) => Promise.race([anim.finished.catch(() => {}), sleep(ms)]);
...
async function switchToPage(pageId, dir = 0) {
  ...
  closeFolderView(); // 別のページのフォルダを開いたまま残さない
  renderPageBar();

  // フェードアウトは fill:'forwards' で「消えたまま」保つが、フェードインを始めた時点で
  // 必ず cancel() する。放置すると合成順で最後まで生き残り、切り替えるたびにグリッドが
  // 透明・横ずれのまま固まる(=切り替わっていないように見える)
  const leave = quickLinksEl.animate(
    [
  ...
  let enter = null;
  try {
    await settleAnim(leave, 200);
    await loadShortcuts();
    enter = quickLinksEl.animate(
      [
        { transform: `translateX(${dir * SWIPE_SLIDE}px)`, opacity: 0 },
        { transform: 'translateX(0)', opacity: 1 },
      ],
    ...
    leave.cancel();
    await settleAnim(enter, 260);
  } finally {
    // 見た目が戻ることをアニメーションの完走に頼らない。タブが裏に回っていると
    // 進行しないまま「running」で固まり、フェードイン開始時点(=透明)を保ち続けるため、
    // 最後は必ず両方 cancel して素のスタイル(見えている状態)へ戻す
    leave.cancel();
    enter?.cancel();
...
// スワイプを受け付けない場所(文字入力中とポップアップの上)
const swipeExcluded = (target) =>
  !!target?.closest?.(
    'input, textarea, [contenteditable="true"], .grid-popup, .shortcut-modal, .icon-picker, .icon-picker-backdrop, .folder-view-backdrop'
  );

// トラックパッドの横スワイプ(precisionタッチパッドはwheelイベントのdeltaXとして届く)。
// #quick-links だけに張っていると、アイコンが少ないときに反応する範囲がほとんど無いので
// スタート画面全体(document)で受ける
let wheelCooldown = false;
document.addEventListener(
  'wheel',
  (e) => {
    if (Math.abs(e.deltaX) < 24 || Math.abs(e.deltaX) <= Math.abs(e.deltaY)) return;
    if (swipeExcluded(e.target)) return;
    e.preventDefault();
...
document.addEventListener(
  'touchstart',
  (e) => {
    if (e.touches.length !== 1 || swipeExcluded(e.target)) return;
...
// フォルダの追加・編集(名前+アイコン)。アイコンの既定は「中身のアイコンを2x2で並べたプレビュー」
function openFolderModal(existing) {
  ...
  title.textContent = existing ? 'フォルダを編集' : 'フォルダを追加';
  modal.appendChild(title);

  const nameInput = document.createElement('input');
  nameInput.className = 'search';
  nameInput.type = 'text';
  nameInput.placeholder = '名前(空欄なら「新しいフォルダ」)';
  nameInput.value = existing?.title ?? '';
  modal.appendChild(nameInput);

  // pendingIcon: undefined=変更なし / null=既定(中身のプレビュー)に戻す / {type,value}=変更
  let pendingIcon;
  ...
  function renderIconPreview() {
    ...
    tileIconContent(iconPreview, existing ?? { id: null, type: 'folder', title: '', icon: null }, pendingIcon);
    const effective = pendingIcon !== undefined ? pendingIcon : existing?.icon ?? null;
    iconLabel.textContent = effective ? 'カスタムアイコン' : '既定(中身のプレビュー)';
  }
  iconBtn.addEventListener('click', () => {
    window.roopieIconPicker.open({
      resetLabel: '既定に戻す(中身のプレビュー)',
      onPick: (icon) => {
        pendingIcon = icon;
        renderIconPreview();
      },
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    if (existing) {
      const patch = { title: name || existing.title };
      if (pendingIcon !== undefined) patch.icon = pendingIcon;
      window.roopieInternal.updateShortcut(existing.id, patch);
    } else {
      await window.roopieInternal.addBookmarkFolder(currentPageId, name, pendingIcon ?? null);
    }
    await loadShortcuts();
    close();
  });
  ...
  function onKeydown(e) {
    if (document.querySelector('.icon-picker')) return; // アイコンピッカーが上にあるときはそちらが先
    if (e.key === 'Escape') close();
    else if (e.key === 'Enter' && e.target === nameInput) saveBtn.click();
  }
