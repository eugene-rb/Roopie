...
class Bookmarks {
  ...
  // iconはスタート画面のタイル用(null = 既定 = 中身のアイコンを並べたプレビュー)
  addFolder(parentId, title, icon) {
    const dest = parentId ?? null;
  ...
  addStartPage(title) {
    const root = this.ensureStartFolder();
    // 既定名は空いている一番小さい番号にする。枚数+1 にすると、途中のページを削除してから
    // 追加したときに既存と同じ名前になり、スタート画面に同名のページが並んでしまう
    const used = new Set(this.children(root.id).map((b) => b.title));
    let number = 1;
    while (used.has(`ページ${number}`)) number++;
    const page = {
      id: crypto.randomUUID(),
      type: 'folder',
      parentId: root.id,
      title: (title ?? '').trim().slice(0, MAX_TITLE) || `ページ${number}`,
      createdAt: Date.now(),
    };
