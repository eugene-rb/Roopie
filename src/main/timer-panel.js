...
class TimerPanel {
  ...
  // **📌で固定したものは待機中(idle)でも出す**——出ないとフロートから始められず、
  // 「📌したのに何も起きない」ことになる(アラームは時刻が来るまで動かないので対象外)
  activeFor(t) {
    if (t.ringing) return true;
    if (t.type === 'clock') return !!t.float && t.status === 'running';
    if (t.float) return t.status !== 'finished';
  ...
  destroy() {
    if (!this.view) return;
    // ウィンドウ破棄後はcontentViewを触れないので、webContentsを閉じるだけにする
    if (!this.window.isDestroyed()) this.window.contentView.removeChildView(this.view);
    // 破棄の順序によっては webContents が先に消えて undefined になる
    const wc = this.view.webContents;
