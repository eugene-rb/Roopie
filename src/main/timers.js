...
class Timers {
  ...
  registerGrace(timerId, fireId, graceMs, run, dangerous) {
    ...
    // 猶予はfire()のchanged()より後に登録されるため、ここで通知しないと画面側に
    // fireId/graceEndsAtが届かない(「あと○秒」が出ず、止めるボタンが猶予キャンセルにならない)。
    // 猶予は永続対象ではないので保存はせず通知のみ
    this.onChange?.();
    return {
      cancel: () => {
  ...
  acknowledge(id) {
    ...
    // 危険アクション(シャットダウン等)の猶予中は、これでは取り消さない。
    // 取り消しは fireId を伴う cancelFire() だけに任せる(「止める」を押しただけで
    // 予定していたシャットダウンが黙って消えるのを防ぐ)
    if (this.hasDangerousGrace(id)) return;
    this.clearGraceFor(id);
    this.settleAfterRing(id);
  }

  hasDangerousGrace(timerId) {
    for (const entry of this.graces.values()) {
      if (entry.timerId === timerId && entry.dangerous) return true;
    }
    return false;
  }
...
function sanitize(input, existing) {
  ...
  if (type === 'countdown' || type === 'clock') {
    ...
    t.actions = {
      ...
      // シャットダウン前の通知(=15秒の猶予とフローティング表示)。OFFなら猶予なしで即実行。
      // 未指定は既存値、それも無ければ通知あり(安全側)
      notifyBeforeShutdown:
        a.notifyBeforeShutdown !== undefined ? !!a.notifyBeforeShutdown : t.actions?.notifyBeforeShutdown ?? true,
    };
  } else {
    t.actions = null;
  }
