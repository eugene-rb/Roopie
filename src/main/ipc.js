...
const { showTabMenu, showTabGroupMenu } = require('./tab-context-menu');
const {
...
function registerIpc() {
  // media-guard-preload が各ドキュメントの開始時に同期で聞く(裏で開いたタブかどうか)。
  // 非同期にすると、ページのスクリプトが先に play() を呼んでしまい塞げない
  ipcMain.on('media-guard:blocked', (e) => {
    e.returnValue = mediaGuard.isBlocked(e.sender);
  });

  // ウィンドウの明暗を**ドキュメントの開始時に同期で**返す。
  // theme:get(非同期)を待つと、ライトのプロファイルでも最初の数十msだけ
  // ダークで描かれてちらつくため(実測で45〜90ms)、preloadから先に聞かせている
  ipcMain.on('theme:window-mode-sync', (e) => {
    const ctx = ctxOf(e);
    e.returnValue = ctx?.incognito ? 'dark' : browser.resolvedWindowMode(bundleOf(e)?.theme.data);
  });
  ...
  // ---- タブグループ(二段タブバー) ----
  ipcMain.on('tab-group:select', (e, groupId) => tabsOf(e)?.selectGroup(groupId));
  ipcMain.on('tab-group:rename', (e, groupId, name) => tabsOf(e)?.renameGroup(groupId, name));
  ipcMain.on('tab-group:new-tab', (e, groupId) => tabsOf(e)?.createTab(undefined, { groupId }));
  // ドラッグ&ドロップの落とし先で決まる所属の付け替え(groupId=null は「グループから外す」)
  ipcMain.on('tab-group:assign', (e, tabId, groupId) => {
    const tabManager = tabsOf(e);
  ...
  ipcMain.on('menu:close', (e) => {
    const ctx = ctxOf(e);
    ctx?.tabManager.showOverlay(false);
    // 翻訳アイコンは選択テキストの翻訳の間だけ出していることがある。畳んだことを伝えて戻させる
    if (ctx && !ctx.window.isDestroyed()) ctx.window.webContents.send('ui:overlay-closed');
  });
  ...
  // サイトの権限の確認(Edge風のドロップダウン)。メイン発だがアンカー(アドレスバーの
  // サイト情報アイコン)の位置はツールバーしか知らないので、測ってから戻してもらう
  ipcMain.on('menu:open-permission', (e, payload) => {
    const ctx = ctxOf(e);
    const tabManager = ctx?.tabManager;
    if (!tabManager?.overlay) return;
    // 測って戻ってくる間に取り下げられていることがある(空のポップアップを残さない)
    if (!browser.hasPendingPermission(ctx)) return;
    const anchor = payload?.anchor;
    tabManager.showOverlay(true);
    tabManager.overlay.webContents.send('perm:show', {
      host: typeof payload?.host === 'string' ? payload.host : '',
      items: Array.isArray(payload?.items) ? payload.items : [],
      // アンカーはウィンドウ座標で届く。オーバーレイは縦タブ時に左へずれるので補正する
      anchor: anchor
        ? { left: anchor.left - tabManager.chromeLeft, right: anchor.right - tabManager.chromeLeft }
        : null,
    });
  });
  ...
  // ---- 翻訳(Edge準拠) ----
  //
  // 差し替えの実務は分離ワールドのpreload(translate-preload.js)。ここは取り次ぎだけ。
  // 訳文の要求は**要求元タブのセッション**で通信する(page-translate.js を参照)

  // 翻訳先に選べる言語の一覧(設定画面の選択肢)
  ipcMain.handle('translate:langs', () => require('./translate').LANGS);
  ipcMain.handle('translate:texts', (e, payload) => pageTranslate.handleTexts(e.sender, payload));
  ipcMain.on('translate:progress', (e, payload) => pageTranslate.handleProgress(e.sender, payload));
  ipcMain.on('translate:page-info', (e, payload) => pageTranslate.handlePageInfo(e.sender, payload));

  // アドレスバーの翻訳アイコンにぶら下げるドロップダウン。アンカーの位置はツールバーしか
  // 知らないので、サイトの権限と同じく測ってから戻してもらう
  ipcMain.on('menu:open-translate', (e, payload) => {
    const ctx = ctxOf(e);
    const tabManager = ctx?.tabManager;
    if (!tabManager?.overlay) return;
    const anchor = payload?.anchor;
    tabManager.showOverlay(true);
    tabManager.overlay.webContents.send('translate:show', {
      ...pageTranslate.popupPayload(ctx, payload?.mode),
      // アンカーはウィンドウ座標で届く。オーバーレイは縦タブ時に左へずれるので補正する
      anchor: anchor
        ? { left: anchor.left - tabManager.chromeLeft, right: anchor.right - tabManager.chromeLeft }
        : null,
  ...
  // ドロップダウンの操作(オーバーレイから)
  ipcMain.on('translate:run', (e, targetLang) => pageTranslate.start(ctxOf(e), null, targetLang));
  ipcMain.on('translate:undo', (e) => pageTranslate.restore(ctxOf(e)));
  ipcMain.on('translate:never-site', (e) => pageTranslate.neverSite(ctxOf(e)));
  ipcMain.on('translate:never-lang', (e, lang) => pageTranslate.neverLang(ctxOf(e), lang));
  ipcMain.on('translate:always-lang', (e, lang, on) => pageTranslate.setAlwaysLang(ctxOf(e), lang, on === true));
  // 選択テキストの翻訳結果は預かりものなので、ドロップダウンを閉じたら捨てる
  ipcMain.on('translate:selection-close', (e) => pageTranslate.clearSelection(ctxOf(e)));
  ...
  // 作ったフォルダを返す(スタート画面はこのIDでタイルを扱うため invoke)
  ipcMain.handle('bookmarks:add-folder', (e, parentId, title, icon) =>
    bundleOf(e)?.bookmarks.addFolder(typeof parentId === 'string' && parentId ? parentId : null, title, icon) ?? null
  );
  ...
  // サイトの権限の確認への返事。'always'(サイトごとに記憶)/ 'once'(今回だけ)/ 'block'(今回限り)
  ipcMain.on('permission:respond', (e, answer) =>
    browser.respondPermissionPrompt(ctxOf(e), answer === 'always' || answer === 'once' ? answer : 'block')
  );
  ...
  // 設定画面用。押した結果(OSへの登録ができたか)まで返す
  ipcMain.handle('default-browser:get', () => ({ isDefault: defaultBrowser.refresh() }));
  ipcMain.handle('default-browser:request', async () => {
    const registered = await defaultBrowser.setAsDefault();
    return { isDefault: defaultBrowser.refresh(), registered };
  });
  ...
  // 無効にしたまま削除・入れ直しをしたときの後始末。拡張機能IDは入れ直しても変わらないので、
  // 「無効にしたもの」の一覧に残ったままだと次の起動でまた外れてしまう
  const forgetDisabled = (e, extensionId) => {
    const bundle = bundleOf(e);
    const ids = bundle?.settings.data.disabledExtensions ?? [];
    if (!bundle || !ids.includes(extensionId)) return;
    bundle.settings.data.disabledExtensions = ids.filter((id) => id !== extensionId);
    bundle.settings.save();
  };
  ipcMain.handle('extensions:install', async (e, extensionId) => {
    ...
    forgetDisabled(e, ext.id);
    browser.sendExtensionsFor(profile.id);
    return { id: ext.id, name: ext.name, version: ext.version };
  });
  // ウェブストア以外の拡張機能をフォルダから読み込む(Chromeの「パッケージ化されていない拡張機能を読み込む」)
  ipcMain.handle('extensions:load-unpacked', async (e) => {
    const ctx = ctxOf(e);
    // シークレットウィンドウには拡張機能を取り付けていない(非永続セッションは非対応)。
    // ここで通すと使用中のプロファイルへ勝手に入ってしまう
    if (!ctx || ctx.incognito) throw new Error('シークレットウィンドウでは拡張機能を読み込めません');
    ...
    const profile = profileOf(e);
    const session = browser.profiles.sessionFor(profile);
    const ext = await browser.extensions.loadUnpacked(session, profile.id, filePaths[0]);
    forgetDisabled(e, ext.id);
    browser.sendExtensionsFor(profile.id);
    return { id: ext.id, name: ext.name, version: ext.version };
  });
  ...
  // ジェスチャーのアクションを実行する(ジェスチャーを受けたページのタブに対して実行する)。
  // 対応するアクションの一覧は gestures.js の ACTIONS
  const performGesture = (wc, action) => {
    ...
    if (!bundle?.gestures.data.enabled || !tabManager || !ctx) return;
    const tab = tabManager.tabs.find((t) => t.view.webContents === wc);
    // ジェスチャーで開くタブはアクティブなタブのすぐ右へ(Chrome/Vivaldi と同じ)
    const open = (url) => tabManager.createTab(url, { nearActive: true });
  ...
  // ジェスチャーの検出はメインプロセス(gesture-input.js)が行うため、そこから直接呼べるようにする
  // (browser.store と同じ受け渡し方。ipc.js は require の下流なので、逆にrequireできない)
  browser.performGesture = performGesture;
  ipcMain.on('gestures:perform', (e, action) => performGesture(e.sender, action));
  ...
  ipcMain.on('settings:set', (e, key, value) => {
    ...
    if (key === 'toolbarItems') {
      ...
      settings.data[key] = sitePermissions.normalizeGrants(value);
    } else if (key === 'weatherLocation') {
      settings.data[key] = normalizeWeatherLocation(value);
    } else if (key === 'audioTabEffect' || key === 'activeTabEffect' || key === 'groupTabEffect') {
      settings.data[key] = browser.AUDIO_TAB_EFFECTS.includes(value) ? value : browser.DEFAULT_SETTINGS[key];
    } else if (/^(audio|active|group)TabEffectColor2?$/.test(key)) {
      // 空文字は「自動(アクセント色)」の意味。それ以外は #rrggbb だけ通す
      settings.data[key] = /^#[0-9a-fA-F]{6}$/.test(value) ? value : '';
    } else if (key === 'translateTargetLang') {
      settings.data[key] = require('./translate').normalizeLang(value);
    } else if (key === 'translateNeverSites') {
      settings.data[key] = pageTranslate.normalizeHosts(value);
    } else if (key === 'translateAlwaysLangs' || key === 'translateNeverLangs') {
      settings.data[key] = pageTranslate.normalizeLangs(value);
    } else if (key === 'startIconSize' || key === 'tabMinWidth') {
      const [min, max] = key === 'startIconSize' ? browser.START_ICON_SIZE_RANGE : browser.TAB_MIN_WIDTH_RANGE;
    ...
    if (key === 'activatePreviousTabOnClose') browser.applyActivatePreviousTabFor(profileId);
