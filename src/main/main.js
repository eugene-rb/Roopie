...
const { urlFromArgv } = require('./url-args');

// 既定のブラウザとして他アプリから開かれたときのURL(`"Roopie.exe" "%1"`)
...
if (!app.requestSingleInstanceLock()) {
  ...
  app.on('second-instance', (_event, argv) => {
    // 既定のブラウザになっていると、他アプリのリンククリックはこちらへ来る。
    // 引数のURLを開くところまでやらないと「Roopieが前に出るだけで何も開かない」ことになる
...
app.whenReady().then(() => {
  ...
  browser.openStartupWindows({ url: appState.takeStartupUrl(), externalUrl: launchUrl });
