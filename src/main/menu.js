  ...
  const template = [
    {
      label: 'ファイル',
      submenu: [
        // ショートカット(メニュー)から開くタブはアクティブなタブのすぐ右へ
        { label: '新しいタブ', accelerator: accel('newTab'), click: () => tabs()?.createTab(undefined, { nearActive: true }) },
        {
          label: '新しいウィンドウ',
    ...
    },
    {
      ...
      submenu: [
        ...
        {
          // 訳し終えているときは元の言語へ戻す(Edgeのアイコンと同じ挙動)
          label: 'ページを翻訳',
          accelerator: accel('translatePage'),
          click: () => require('./page-translate').toggle(windows.focused()),
        },
        { type: 'separator' },
        {
          label: 'アドレスバーにフォーカス',
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          click: () => tabs()?.createTab('roopie://bookmarks', { nearActive: true }),
        },
      ],
    },
    {
      label: '履歴',
      submenu: [
        { label: '履歴を表示', accelerator: accel('history'), click: () => tabs()?.createTab('roopie://history', { nearActive: true }) },
        {
          label: 'ダウンロード',
          accelerator: accel('downloads'),
          click: () => tabs()?.createTab('roopie://downloads', { nearActive: true }),
        },
      ],
    },
    {
      ...
      submenu: [
        {
          ...
          click: () => tabs()?.createTab('roopie://settings', { nearActive: true }),
        },
      ],
    },
  ];
