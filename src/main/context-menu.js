...
function attachContextMenu(webContents, tabManager) {
  const listener = (_event, params) => {
    ...
    if (params.selectionText) {
      ...
      // 選択テキストの翻訳(Edge風)。訳文はアドレスバーの翻訳アイコンにぶら下がる
      // ドロップダウンに出す(選択位置に出すにはページ→ウィンドウ座標の変換が要るため)
      if (pageTranslate.canTranslate(ownTab)) {
        const targetName = translate.langName(translate.normalizeLang(bundle?.settings.data.translateTargetLang));
        add({
          label: `選択したテキストを${targetName}に翻訳`,
          click: () => pageTranslate.translateSelection(ownCtx, webContents, text),
        });
      }
    ...
    if (!params.linkURL && !params.selectionText && !params.isEditable && params.mediaType === 'none') {
      ...
      // ページ翻訳(Edge風)。訳し終えているときは「元の言語を表示」に変わる
      if (pageTranslate.canTranslate(ownTab)) {
        const translated = ownTab?.translate?.state === 'done';
        const targetName = translate.langName(translate.normalizeLang(bundle?.settings.data.translateTargetLang));
        add({
          label: translated ? '元の言語を表示' : `${targetName}に翻訳`,
          click: () => pageTranslate.toggle(ownCtx, ownTab?.id),
        });
      }
