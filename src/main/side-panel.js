...
class SidePanel {
  ...
  destroyWebView() {
    ...
    if (!this.window.isDestroyed()) this.window.contentView.removeChildView(this.webView);
  ...
  destroyPanelView() {
    if (!this.panelView) return;
    if (!this.window.isDestroyed()) this.window.contentView.removeChildView(this.panelView);
    const wc = this.panelView.webContents;
    if (wc && !wc.isDestroyed()) wc.close();
    this.panelView = null;
  }
