...
## 追加修正: トークン差し替えの影響範囲（`d8ee6d1`）

ステップ2の「`--card` を差し替えれば全部揃う」は差分が小さくて綺麗だが、
**影響範囲がそのまま広い**という裏返しがある。`var(--card)` / `var(--card-hover)` の
参照は全部で **62箇所**あり、そのうち「浮遊して下を隠すのが仕事の面」まで
0.1 の半透明にしてしまっていた。

該当は `.icon-picker` / `.tr-more-menu` / `.panel-menu` / `.modal` の4つ
（いずれも内部ページ専用でchrome側では未使用）。後ろの内容が透けて読めた。
モーダルは `.modal-backdrop`（黒50%）の上に乗るため、薄いままだと
**自分の暗幕と見分けがつかない**という別の問題もあった。

→ `--glass-menu` を足し、ガラスの質感（ぼかしと縁）は残したまま濃さだけ戻した。
既定は `--menu-bg`（不透明）、glass 時は `--c-menu` 88%。

**教訓**: トークン差し替えで一括適用したときは、そのトークンの参照箇所を
`grep -n 'var(--card)'` で必ず**列挙**すること。スクショに写る要素だけ見ても、
閉じているドロップダウンやモーダルの欠陥は見えない。

## 残っている確認事項（既知の欠陥ではなく未測定）

- `.modal` / `.icon-picker` を開いた状態のスクショは未取得。`gen-screenshots.js` は
  どれも開かないため。上の修正で88%まで戻したので読めるはずだが目視はしていない。
- `background-attachment: fixed` は Chromium のスクロール再描画で重くなりうる。
  今それが乗っているのは設定と履歴＝**最も長くスクロールするページ**で、
  さらに上にぼかしたカードが乗る。`roopie://history` を glass でスクロールして
  カクつくようなら、`body::before` の固定レイヤーに逃がす（背景伝播とスタッキングの
  理由で一度は見送った書き方だが、`fixed` のコストが高いときの逃げ道になる）。
- ミニプレイヤーは **白いページの上**で見ること（MDN/Wikipedia など）。
  `--text` は明るい灰色なので、`--c-menu` 55% が白背景に乗ると中間灰になり
  コントラストが際どくなる。洗い流されていたら混合率を上げる。
