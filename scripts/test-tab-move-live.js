...
app.whenReady().then(async () => {
  ...
  try {
    ...
    // 休止中(まだ読み込んでいない)タブ = セッション復元と同じ hibernate 経路。
    // 裏で開いただけのタブは読み込まれるので、ここは明示的に hibernate を指定する
    const dormant = ctxA.tabManager.createTab(dormantUrl, {
      background: true,
      hibernate: true,
      initialTitle: '休止ページ',
    });
    ...
    const crossDormant = ctxA.tabManager.createTab(crossDormantUrl, { background: true, hibernate: true });
    await sleep(300);
