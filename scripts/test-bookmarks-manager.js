...
function testMainLogic() {
  ...
  // スタート画面のページの既定名。途中のページを削除してから追加しても名前が衝突しない
  // (スタート画面はページ名をそのまま並べて表示するので、同名だとどれがどれか分からなくなる)
  const p2 = bm.addStartPage('');
  const p3 = bm.addStartPage('');
  check('ページの既定名は連番', [p2.title, p3.title], ['ページ2', 'ページ3']);
  bm.remove(p2.id);
  const p4 = bm.addStartPage('');
  check('削除後に追加しても既存のページ名と重複しない', p4.title, 'ページ2');
  check('スタート画面のページ名は重複しない', new Set(bm.startPages().map((p) => p.title)).size, bm.startPages().length);
  bm.remove(p3.id);
  bm.remove(p4.id);
  ...
  // フォルダにもカスタムアイコンを持たせられる(スタート画面のタイル用。既定はnull=中身のプレビュー)
  const page = bm.startPages()[0];
  check('フォルダのアイコンの既定はnull', bm.addFolder(page.id, 'アイコン無し').icon, null);
  const iconFolder = bm.addFolder(page.id, '絵文字', { type: 'emoji', value: '🎨' });
  check('作成時にアイコンを指定できる', iconFolder.icon, { type: 'emoji', value: '🎨' });
  check('不正なアイコンは既定(null)になる', bm.addFolder(page.id, '不正', { type: 'zzz' }).icon, null);
  bm.updateItem(iconFolder.id, { title: '絵文字2', icon: { type: 'emoji', value: '🔥' } });
  check('フォルダの名前とアイコンを後から変えられる', [iconFolder.title, iconFolder.icon], [
    '絵文字2',
    { type: 'emoji', value: '🔥' },
  ]);
  bm.updateItem(iconFolder.id, { icon: null });
