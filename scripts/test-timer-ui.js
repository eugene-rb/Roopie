...
app.whenReady().then(async () => {
  try {
    ...
    // フロートの時計が進むか(止まっていると「動かない」ように見える)
    const floatTime = (type) =>
      js(fWc, `document.querySelector('.timerp-row[data-type="${type}"] .timerp-time')?.textContent ?? null`);
    const cdBefore = await floatTime('countdown');
    const swBefore = await floatTime('stopwatch');
    await sleep(2200);
    const cdAfter = await floatTime('countdown');
    const swAfter = await floatTime('stopwatch');
    console.log(`   カウントダウン: ${cdBefore} → ${cdAfter} / ストップウォッチ: ${swBefore} → ${swAfter}`);
    ...
    ctx.window.minimize();
    await sleep(3000);
    const swHidden = await floatTime('stopwatch');
    ctx.window.restore();
    await sleep(400);
    console.log(`   最小化中: ${swAfter} → ${swHidden}`);
    check('最小化中もフロートの時計が進む', swAfter !== swHidden, true);
    ...
    // 📌で固定したものは待機中(idle)でもフロートに出て、そこから始められる。
    // 出ないと「📌したのに何も起きない = フロートのタイマーが動かない」ことになる
    const idleSw = bundle.timers.add({ type: 'stopwatch', name: '待機中SW', float: true });
    await sleep(500);
    check('(前提)作った直後は待機中', bundle.timers.find(idleSw.id).status, 'idle');
    check(
      '📌した待機中のタイマーもフロートに出る',
      await js(fWc, `!!document.querySelector('.timerp-row[data-type="stopwatch"] .timerp-name')` +
        ` && [...document.querySelectorAll('.timerp-name')].some((n) => n.textContent === '待機中SW')`),
      true
    );
    check(
      '待機中は開始ボタン(▶)になる',
      await js(fWc, `(() => {
        const row = [...document.querySelectorAll('.timerp-row')].find((r) => r.querySelector('.timerp-name')?.textContent === '待機中SW');
        return !!row?.querySelector('.timerp-circle.paused');
      })()`),
      true
    );
    await js(fWc, `(() => {
      const row = [...document.querySelectorAll('.timerp-row')].find((r) => r.querySelector('.timerp-name')?.textContent === '待機中SW');
      row.querySelector('.timerp-circle').click();
      return true;
    })()`);
    await sleep(500);
    check('フロートから待機中のタイマーを開始できる', bundle.timers.find(idleSw.id).status, 'running');
    bundle.timers.remove(idleSw.id);
    await sleep(300);
