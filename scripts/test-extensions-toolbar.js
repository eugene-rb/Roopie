...
function makeFixture(name, target, key) {
  const dir = target ?? fs.mkdtempSync(path.join(os.tmpdir(), 'roopie-ext-act-'));
...
// ピン留めアイコンの置かれ方(ツールバーの他のボタンと揃っているか)を実測する。
// 見た目はスクショではなく矩形で確かめる(このハーネスのスクショは古いフレームを返すことがある)
const layoutState = (wc) =>
  js(
    wc,
    `(() => {
       ...
       const rect = (el) => {
         ...
         return { x: round(r.x), y: round(r.y), w: round(r.width), h: round(r.height), cy: round(r.y + r.height / 2) };
       };
       const list = document.getElementById('toolbar-extensions');
       const shown = list?.shadowRoot
         ? [...list.shadowRoot.querySelectorAll('.action')].filter((n) => n.style.display !== 'none')
         : [];
       return {
         icons: shown.map(rect),
         list: rect(list),
         puzzle: rect(document.getElementById('extensions-menu-btn')),
         downloads: rect(document.getElementById('downloads-btn')),
       };
     })()`
  );

// 前回インストール済みの拡張として、実物と同じ配置でディスクに置いておく
// (<userData>/profiles/<id>/extensions/<拡張ID>/<version>_0/)。
// ローダーは manifest.key のあるものだけを「インストール済み」として読み込むため、
// ウェブストア版と同じく公開鍵を持たせる
function seedInstalled(profileId, name) {
  const key = crypto
    .generateKeyPairSync('rsa', { modulusLength: 2048, publicKeyEncoding: { type: 'spki', format: 'der' } })
    .publicKey.toString('base64');
  makeFixture(name, path.join(tmp, 'profiles', profileId, 'extensions', 'seeded', '1.0.0_0'), key);
}
...
app.whenReady().then(async () => {
  try {
    registerIpc();
    browser.initData();
    // 起動時の読み込み(非同期)は、ウィンドウの初期状態送信より後に終わることがある。
    // 完了後に配り直さないと一覧が空のまま届き、パズルボタンが出ないままになる
    seedInstalled(browser.profiles.active().id, '保存済み拡張');
    // 実機(拡張が複数)ではディスクからの読み込みがウィンドウの初期状態送信より後に終わる。
    // 検証でも同じ順序を必ず作るため、最初の読み込みだけ遅らせる
    // (拡張機能システムの用意そのものは実機と同じくウィンドウ表示前に済ませる)
    const profileSession = browser.profiles.sessionFor(browser.profiles.active());
    const realLoad = profileSession.extensions.loadExtension.bind(profileSession.extensions);
    profileSession.extensions.loadExtension = async (...a) => {
      profileSession.extensions.loadExtension = realLoad; // 遅らせるのは起動時の1回だけ
      await sleep(2500);
      return realLoad(...a);
    };
    ...
    const atStartup = await toolbarState(uiWc);
    console.log('   起動直後:', JSON.stringify(atStartup));
    check('起動時: 保存済み拡張が読み込まれる', profileSession.extensions.getAllExtensions().length, 1);
    check('起動時: 保存済み拡張があればパズルボタンが出る', atStartup.puzzleHidden, false);
    ...
    // 配置: 設定画面の一覧と同じidを使っていた頃は、あちら向けの margin-top が当たって
    // アイコンだけ14px下へずれていた。隣のボタンと中心・大きさ・間隔が揃うかを実測で見る
    const layout = await layoutState(uiWc);
    console.log('   配置:', JSON.stringify(layout));
    check('アイコンの中心がツールバーの他のボタンと揃う', layout.icons[0]?.cy, layout.puzzle.cy);
    check('アイコンの大きさが他のツールバーボタンと同じ', [layout.icons[0]?.w, layout.icons[0]?.h], [30, 30]);
    check(
      'パズルボタンとの間隔が他のボタン同士と同じ',
      Math.round(layout.puzzle.x - (layout.list.x + layout.list.w)),
