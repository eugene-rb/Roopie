...
app.whenReady().then(async () => {
  ...
  // ホイールクリック(中クリック)→ 裏で開く。
  // ※このタイミングでタブを2枚以上にすると sendInputEvent がページに届かなくなる
  //   (ハーネス側の制約)。ホイールクリックで開いたタブの**並び順**の検証は
  //   scripts/test-window-tabs.js 側(同じ setWindowOpenHandler 経由)で行っている
  ...
  check('ホイールクリックのタブにはリンク元が記録される(並び順の基準)', opened.openerTabId, first.id);
  ...
  // 裏で開いたタブは読み込む。ただし選ぶまでメディアの再生は始めさせない(media-guard)。
  // 再生を塞げているかどうかの検証は scripts/test-autoplay-policy.js
  await Promise.race([
    new Promise((resolve) => opened.view.webContents.once('did-finish-load', resolve)),
    sleep(8000),
  ]);
  check('裏で開いたタブも読み込まれる', opened.view.webContents.getURL().endsWith('/other'), true);
  check('休止扱いにはならない', opened.hibernated, false);
  check('自動再生を塞ぐ印が立つ', opened.mediaGuarded, true);
  ...
  // 裏のタブへは手動で切り替えられる → その時点で再生の抑止が解ける
  ...
  check('切り替えると自動再生の抑止が解ける', opened.mediaGuarded, false);
  ...
  // ---- 休止中タブ(セッション復元と同じ hibernate)の仮タイトル。このハーネスは
  //      chrome UIのHTMLを読み込んでいないため、DOMではなくtabs:stateの送信内容そのものを見る ----
  let lastState = null;
  ...
  const dormant = tabManager.createTab(`http://localhost:${PORT}/other`, { background: true, hibernate: true });
  await sleep(300);
  const dormantState = lastState?.tabs.find((t) => t.id === dormant.id);
  check('hibernateで開いたタブは読み込まない', dormant.view.webContents.getURL(), '');
