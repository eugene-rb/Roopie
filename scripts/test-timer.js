...
app.whenReady().then(async () => {
  ...
  // 猶予の情報が画面側へ届くこと(届かないと「あと○秒」が出ず、止めるボタンが
  // cancelFireではなくacknowledgeになって危険アクションが黙って消える)
  const ringingRow = timers.list().find((t) => t.id === t3.id);
  check('猶予中のfireIdが一覧に載る', ringingRow.fireId, fired.fireId);
  check('猶予中のgraceEndsAtが一覧に載る', typeof ringingRow.graceEndsAt, 'number');

  // 「止める」(acknowledge)では危険アクションを取り消さない
  timers.acknowledge(t3.id);
  check('acknowledgeでは危険アクションの猶予を消さない', timers.graces.has(fired.fireId), true);
  check('acknowledge後もringingのまま', timers.find(t3.id).status, 'ringing');
  ...
  // ---- 「シャットダウン前に通知を出す」OFF: 猶予を挟まず即実行(dry-runなので実機は落ちない) ----
  const t4 = timers.add({
    type: 'countdown',
    durationMs: 5000,
    name: '通知なしシャットダウン',
    actions: { shutdown: true, shutdownConfirmed: true, notifyBeforeShutdown: false },
  });
  check('通知の設定が保存される', timers.find(t4.id).actions.notifyBeforeShutdown, false);
  timers.start(t4.id);
  timers.tick(Date.now() + 10_000);
  check('通知OFFは猶予を作らない', timers.hasDangerousGrace(t4.id), false);
  check('通知OFFは発火と同時に決着する', timers.find(t4.id).status, 'finished');
  timers.remove(t4.id);

  // 既定(未指定)は通知あり=猶予つき
  const t5 = timers.add({ type: 'countdown', durationMs: 5000, actions: { shutdown: true, shutdownConfirmed: true } });
  check('通知の既定は「出す」', timers.find(t5.id).actions.notifyBeforeShutdown, true);
  timers.start(t5.id);
  timers.tick(Date.now() + 10_000);
  check('通知ONは猶予を作る', timers.hasDangerousGrace(t5.id), true);
  timers.remove(t5.id);
  ...
  // dry-runガードは開発時のみ有効(製品版では環境変数でも無効化されない)
  check('dry-runガードが効いて未実行を返す', timerActions.runShutdown(), false);
